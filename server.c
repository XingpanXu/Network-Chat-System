...
int main()
{
    ...
    while (1)
    {
        ...
        // 针对当前套接字创建一个线程，对当前套接字的留言进行处理
        // pthread_create(malloc(sizeof(pthread_t)), NULL, (void *)(&send_leave_msg), (void *)i);
    }
    return 0;
}
...
void init()
{
    ...
    char buf1[20];
    FILE *fp1 = NULL;
    fp1 = fopen("messages.txt", "r");
    while (fscanf(fp1, "%s", buf1) != EOF)
    {
        strcpy(messages[message_count].from_name, buf1);
        fscanf(fp1, "%s", buf1);
        strcpy(messages[message_count].to_name, buf1);
        fscanf(fp1, "%s", buf1);
        strcpy(messages[message_count].data, buf1);
        message_count++;
    }
    fclose(fp1);
}
...
void save_messages()
{
    ...
    {
        strcpy(buf, messages[i].from_name);
        strcat(buf, "\n");
        fprintf(fp, buf);
        strcpy(buf, messages[i].to_name);
        strcat(buf, "\n");
        fprintf(fp, buf);
        strcpy(buf, messages[i].data);
        strcat(buf, "\n");
        fprintf(fp, buf);
    }
    fclose(fp);
...
/*用户留言*/
void leave_messages(char *username, char *data, int sfd)
{
    int i, j;
    time_t now;
    char send_man[20];
    char nowtime[20], temp[30];
    now = time(NULL);
    time(&now);
    struct tm *tempTime = localtime(&now);
    strftime(nowtime, 20, "[%H:%M:%S]", tempTime);
    ...
    for (i = 0; i < MAXMEM; i++)
    {
        if (strcmp(username, users[i].username) == 0)
        {
            char details[BUFFSIZE];
            strcpy(details, nowtime);
            strcat(details, ":");
            strcat(details, data);
            strcpy(messages[message_count].from_name, send_man);
            strcpy(messages[message_count].to_name, username);
            strcpy(messages[message_count].data, details);

            strcpy(temp, "Message succeeded.\n");
            write(connfd[sfd], temp, strlen(temp) + 1);

for (int k = 0; k <message_count; k++)
{
    printf("(%s%s%s)",messages[k].from_name,messages[k].to_name,messages[k].data);
}
    ...
    char buf[BUFFSIZE];
    strcpy(buf, "User does not exist.\n");
    write(connfd[sfd], buf, strlen(buf) + 1);
    return;
}
/*发送留言*/
void send_leave_msg(int sfd)
{
    int i, j;
    char to_name[20];
    for (j = 0; j < MAXMEM; j++)
    {
        if (sfd == online_users[j].socketfd)
        {
            strcpy(to_name, online_users[j].username);
            break;
        }
    }
    for (int i = 0; i < message_count; i++)
    {
        if (strcmp(to_name, messages[i].to_name) == 0)
        {
            char buf[BUFFSIZE];

            strcpy(buf, "messages ");
            strcat(buf, "from ");
            strcat(buf, messages[i].from_name);
            strcat(buf, ": ");
            strcat(buf, messages[i].data);
            strcat(buf, "\n");
            write(connfd[sfd], buf, strlen(buf) + 1);
        }
    }
